...
// ✅ IMPORTANT: normalize whatever SelectCompFunctional returns
const normalizeSelectValue = (input) => {
  // event object
  if (input && typeof input === "object" && "target" in input && input.target?.value != null) {
    return String(input.target.value);
  }

  // {label,value} object
  if (input && typeof input === "object" && input.value != null) {
...
export default function Listings({
  ...
  // ✅ store actual sort VALUE (mileage_desc etc)
  ...
  const getStatusBadge = (vehicle) => {
    ...
    const statusVal = String(vehicle?.status || "").trim().toLowerCase();

    if (booking === "temporary booked") return { text: "Reserved", color: "#dc3545" };
    if (statusVal === "sale") return { text: "Sale", color: "#198754" };
  ...
  /**
   * ✅ UPDATED SORT HANDLER (fix your issue)
   * Accepts label or value or event, then converts to API value.
   */
  const handleSortChange = (input) => {
    const raw = normalizeSelectValue(input).trim();

    // If select returns value already (like "mileage_desc")
    const byValue = SORT_OPTIONS.find((o) => o.value === raw);
    if (byValue) {
      setSort(byValue.value);
      goToPage(1);
      return;
    }

    // If select returns label (like "Mileage High to Low")
    const byLabel = SORT_OPTIONS.find((o) => o.label === raw);
    setSort(byLabel?.value || "");
    goToPage(1);
  };
  ...
  // reset page when filters/endpoints change (safe)
  useEffect(() => {
    setPage(1);
  }, [endpoint]);
      ...
      <div className="boxcar-container">
        ...
        <div className="row">
          ...
          <div className="wrap-sidebar-dk side-bar col-xl-3 col-md-12 col-sm-12">
            <div className="sidebar-handle">Show Filter</div>
            ...
            <div className="inventory-sidebar">
              <div className="inventroy-widget widget-location">
                <div className="row">
                  ...
                  {/* keep your other sidebar fields here */}
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="text-box">
                ...
                <form onSubmit={(e) => e.preventDefault()} className="d-flex">
                  <div className="form_boxes v3">
                    {/* ✅ we still pass LABELS for display (no need to change your component) */}
                    <SelectCompFunctional
                      options={SORT_OPTIONS.map((o) => o.label)}
                      value={sortLabelFromValue(sort)}
                      onChange={handleSortChange} // ✅ now works with label/value/event
                    />
                  </div>
                  ...
                  <div className="form_boxes v3 ms-3">
                    ...
                    <SelectCompFunctional
                      ...
                      value={hydrated ? String(perPage) : "50"}
